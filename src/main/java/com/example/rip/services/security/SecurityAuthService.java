...
public class SecurityAuthService {
    ...
    public ResponseEntity<?> createAuthToken(@RequestBody JwtRequest authRequest) {
        ...
        String varStr = userDetails.getAuthorities().toString();
        if(varStr.contains("ROLE_ADMIN")){
    ...
    public ResponseEntity<?> resetToken(String username) {
        ...
        String varStr = userDetails.getAuthorities().toString();
        if(varStr.contains("ROLE_ADMIN")){
            return ResponseEntity.ok(new JwtResponse(token, username, "ADMIN"));
        }else {
            return ResponseEntity.ok(new JwtResponse(token, username, "USER"));
        }
    }
}
